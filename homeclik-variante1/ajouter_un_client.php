<?php
    ...
    if($_SERVER['REQUEST_METHOD']=='POST'){
        if(isset($_POST['nom'])&&isset($_POST['prenom'])&&isset($_POST['code_pays'])&&isset($_POST['telephone_portable'])&&isset($_POST['telephone_fixe'])&&isset($_POST['num_voie'])&&isset($_POST['nom_voie'])&&isset($_POST['num_apt'])&&isset($_POST['code_postal'])&&isset($_POST['ville'])&&isset($_POST['date_rdv'])){
            ...
            $now = new DateTime();
            if($date_rdv<$now){
                echo '<h1><img src="resources/bad.png" width="5%" height="auto" /> La date n\'est pas dans le passé, merci de bien vouloir réessayer !</h1>';
                die();
            }
            ...
            if($conn->affected_rows==0){
                echo '<script>Swal.fire({title: "Erreur",text: "Ce client existe déjà, merci d\'en choisir un autre",icon: "error",confirmButtonText: "OK"}).then(function() {window.location.href = "ajouter_un_client.php";});</script>';
            }else if($conn->affected_rows==1){
                echo '<script>Swal.fire({title: "Client ajouté",text: "Le client a bien été ajouté",icon: "success",confirmButtonText: "OK"}).then(function() {window.location.href = "index.php";});</script>';
                ...
                if($conn->affected_rows==0){
                    echo '<script>Swal.fire({title: "Erreur",text: "Ce client a déjà un rendez-vous pour ce jour-ci, merci de décaler !",icon: "error",confirmButtonText: "OK"}).then(function() {window.location.href = "ajouter_un_client.php";});</script>';
                }else if($conn->affected_rows==1){
                    echo '<script>Swal.fire({title: "Client ajouté",text: "Le rdv du client a bien été ajouté",icon: "success",confirmButtonText: "OK"}).then(function() {window.location.href = "index.php";});</script>';
                }else{
                    echo 'error='.mysqli_error($conn);
                    echo '<h1><img src="resources/bad.png" width="5%" height="auto" /> Un problème est survenu, merci de réessayer !</h1>';
                    echo '<script>Swal.fire({title: "Erreur",text: "Un problème est survenu, merci de réessayer !",icon: "error",confirmButtonText: "OK"}).then(function() {window.location.href = "ajouter_un_client.php";});</script>';
                }
            }else{
                echo 'error='.mysqli_error($conn);
                echo '<script>Swal.fire({title: "Erreur",text: "Un problème est survenu, merci de réessayer !",icon: "error",confirmButtonText: "OK"}).then(function() {window.location.href = "ajouter_un_client.php";});</script>';
